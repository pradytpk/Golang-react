...
import (
	...
	"github.com/pradytpk/go-react-calorie/models"
...
func AddEntry(c *gin.Context) {
	...
	_, inserErr := entryCollection.InsertOne(ctx, entry)
	if inserErr != nil {
		msg := fmt.Sprintf("order item wanot created")
		c.JSON(http.StatusBadRequest, gin.H{"error": msg})
		fmt.Println(inserErr)
		return
	}
	defer cancel()
...
func GetEntryById(c *gin.Context) {
	...
	var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
	var entry bson.M
	if err := entryCollection.FindOne(ctx, bson.M{"_id": EntryID}).Decode(&entry); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		fmt.Println(err)
		return
	}
